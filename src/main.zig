...
pub fn main() anyerror!void {
    ...
    for (tree.rootDecls()) |decl_addr| {
        std.debug.print("-----\n", .{});
        if (try getDocComments(ally, tree, decl_addr)) |dc| {
            defer ally.free(dc);
            std.debug.print("doc comment: {s}\n", .{dc});
            const tag = node_tags[decl_addr];
            // handle if it is a function
            if (tag == .fn_decl) {
                const proto = node_datas[decl_addr].lhs;
                var params: [1]ast.Node.Index = undefined;
                const fnproto = fnProto(
                    tree,
                    proto,
                    &params,
                ).?;
                std.debug.print("sig: {s}\n", .{getFunctionSignature(tree, fnproto)});
            }
            ...
            {
                const vardecl = varDecl(tree, decl_addr).?;
                const vardecl_sig = getVariableSignature(tree, vardecl);
                std.debug.print("sig: {s}\n", .{vardecl_sig});
            }
        }
    }
